export default {
  DefaultOpenStrategy: {
    ...
    description: '',
    lang: 'typescript',
    code: `import { StackNormal, StackLeft, StackRight, StackTop, StackBottom } from '@jood/v-modal';
  ...
  setup() {
    ...
    const onOpen = () => {
      modalService.open({
        ...
        // openStrategy: new StackLeft(), // left to right
        // openStrategy: new StackRight(), // right to left
        // openStrategy: new StackTop(), // top to bottom
        // openStrategy: new StackBottom(), // bottom to top
        floatingMode: true,
        overlayClose: true
      });
    };
  ...
  setup() {
    ...
    const onOpen = () => {
      modalService.open({
        ...
        openStrategy: new CustomOpenStrategy(), // your open strategy
        floatingMode: true,
        overlayClose: true
      });
    };
  ...
  CustomOpenStrategy: {
    title: 'CustomOpenStrategy.ts',
    description: '',
    lang: 'typescript',
    code: `import { OpenStrategy } from '@jood/v-modal';
...
export class CustomOpenStrategy implements OpenStrategy {
  ...
  opened() {
    return {
      pivot: { border: '5px solid #0000ff' }
    };
  }

  closing() {
    return {
      pivot: {  transform: 'scale(0.8, 0.8)', opacity: 0 },
      overlay: { opacity: 0 }
    };
  }
